"""
Embedding pipeline script for Physical AI Textbook - GEMINI VERSION.
Parses markdown chapters, chunks on H2/H3 sections, and sends to RAG backend with Gemini.
"""
...
def embed_chapter(chapter_path: str, api_url: str = "http://localhost:8000"):
    """
    Parse chapter, chunk it, and send to Gemini embedding API.
    """
    print(f"Processing: {os.path.basename(chapter_path)}")
    ...
    try:
        ...
        print(f"  [OK] Embedded successfully with Gemini: {result['message']}")
...
def main():
    """Main embedding pipeline with Gemini."""
    print("=" * 60)
    print("Physical AI Textbook - Embedding Pipeline (Gemini)")
    print("=" * 60)
    print()
    ...
    print(f"Using AI: Google Gemini (FREE!)")
    print()
    ...
    print("=" * 60)
    print("[SUCCESS] Embedding pipeline complete with Gemini!")
    print("=" * 60)
    print()
    print("Next steps:")
    print("  1. Check collection: curl http://localhost:8000/collection/info")
    print("  2. Test query: curl -X POST http://localhost:8000/query \\")
    print("     -H 'Content-Type: application/json' \\")
    print("     -d '{\"query\": \"What is ROS 2?\", \"persona\": \"beginner\"}'")
